...
const props = defineProps<{
  submissionsPagination: SubmissionPagination; 
  filters: InertiaFilters; 
  sorting: InertiaSorting; 
}>();

const submissionsPagination = computed(() => props.submissionsPagination ?? []);
const submissions = ref<Submission[]>(props.submissionsPagination ? props.submissionsPagination.data : []); // Use the 'data' array from the pagination prop
...
const pagination = ref({
    pageIndex: submissionsPagination.value.current_page - 1, // Initialize from props (0-indexed)
    pageSize: submissionsPagination.value.per_page, // Initialize from props
});

// --- Update table data when there is changes ---
watch(() => props.submissionsPagination, (newSubmissionsData) => {        
  submissions.value = newSubmissionsData ? newSubmissionsData.data : [];
  pagination.value.pageIndex = newSubmissionsData ? newSubmissionsData.current_page : 1;
...
const triggerInertiaVisit = (options?: { resetPageIndex?: boolean; newPageIndex?: number }) => {
    ...
    router.get(route('submissions.index'), params,
    {
        ...
        preserveScroll: true,
        replace: true,
        only: ['submissionsPagination', 'filters', 'sorting', 'flash'],
    }
)};
...
const submissionNameToDelete = ref<string>('');
...
const handleDelete = (e: Event) => {
    ...
    form.delete(route('submissions.destroy', { id: form.id, ...params }), {
        preserveScroll: true,
      ...
      return h('div', { class: 'flex space-x-2' }, [
        ...
        h('button', {
          ...
          onClick: () => {
            ...
            submissionNameToDelete.value = item.company_name; 
            form.id = item.id
...
// --- Table Instance ---
const table = useVueTable<Submission>({
        ...
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto ">
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
                <div class="flex items-center space-x-2">
                    ...
                    <button
                        @click="triggerInertiaVisit({ newPageIndex: submissionsPagination.last_page })"
                        :disabled="submissionsPagination.current_page === submissionsPagination.last_page"
                ...
                <div class="flex items-center space-x-2 text-sm text-gray-700">
                    <span>Page</span>
                    <strong>
                        {{ submissionsPagination.current_page }} of {{ submissionsPagination.last_page }}
                    </strong>
                </div>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <input
                        ...
                        @change="e => {
                            const newPage = Math.max(1, Math.min(submissionsPagination.last_page, Number((e.target as HTMLInputElement).value)));
                ...
                <DialogContent>
                    <form class="space-y-6" @submit="handleDelete">
                        <DialogHeader class="space-y-3">
                            <DialogTitle class=" text-lg/8">Are you sure you want to delete <br />"{{ submissionNameToDelete }}"?</DialogTitle>
                            <DialogDescription>
                                Once after deleted the submission, you can't able to do the recovery.
                            </DialogDescription>
                        </DialogHeader>

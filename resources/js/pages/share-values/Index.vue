...
import { 
    ShareValue, 
    ShareValuePagination, 
} from '@/types/share-value';
...
const props = defineProps<{
  shareValuesPagination: ShareValuePagination; 
  filters: InertiaFilters; 
  sorting: InertiaSorting; 
}>();

const shareValuesPagination = computed(() => props.shareValuesPagination ?? []);
const shareValues = ref<ShareValue[]>(props.shareValuesPagination ? props.shareValuesPagination.data : []); // Use the 'data' array from the pagination prop
...
const pagination = ref({
    pageIndex: shareValuesPagination.value.current_page - 1, // Initialize from props (0-indexed)
    pageSize: shareValuesPagination.value.per_page, // Initialize from props
});

// --- Update table data when there is changes ---
watch(() => props.shareValuesPagination, (newShareValuesData) => {        
  shareValues.value = newShareValuesData ? newShareValuesData.data : [];
  pagination.value.pageIndex = newShareValuesData ? newShareValuesData.current_page : 1;
...
const triggerInertiaVisit = (options?: { resetPageIndex?: boolean; newPageIndex?: number }) => {
    ...
    router.get(route('share-values.index'), params,
    {
        preserveState: true,
        preserveScroll: true,
        replace: true,
        only: ['shareValuesPagination', 'filters', 'sorting', 'flash'],
    }
)};
...
// //   router.get(route('shareValues.edit', { id: id }));
// };

const isDeleteDialogOpen = ref(false);
const shareValueNameToDelete = ref<string>('');
...
const columnHelper = createColumnHelper<ShareValue>();
      ...
      return h('div', { class: 'flex space-x-2' }, [
        ...
        h('button', {
          ...
          onClick: () => {
            ...
            // shareValueNameToDelete.value = item.name; 
            // form.id = item.id
...
// --- Table Instance ---
const table = useVueTable<ShareValue>({
    get data() { return shareValues.value; },
    columns: columns,
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
                <div class="flex items-center space-x-2">
                    <button
                        ...
                        :disabled="shareValuesPagination.current_page === 1"
                    ...
                    <button
                        @click="triggerInertiaVisit({ newPageIndex: pagination.pageIndex + 1 })"
                        :disabled="shareValuesPagination.current_page === shareValuesPagination.last_page"             
                        class="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 dark:text-white bg-white dark:bg-gray-700 dark:border-gray-800 hover:bg-gray-50 hover:dark:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Next
                    </button>
                    <button
                        @click="triggerInertiaVisit({ newPageIndex: shareValuesPagination.last_page })"
                        :disabled="shareValuesPagination.current_page === shareValuesPagination.last_page"
                ...
                <div class="flex items-center space-x-2 text-sm text-gray-700">
                    <span>Page</span>
                    <strong>
                        {{ shareValuesPagination.current_page }} of {{ shareValuesPagination.last_page }}
                    </strong>
                </div>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <input
                        ...
                        @change="e => {
                            const newPage = Math.max(1, Math.min(shareValuesPagination.last_page, Number((e.target as HTMLInputElement).value)));
                ...
                <DialogContent>
                    ...
                    <form class="space-y-6" >
                        <DialogHeader class="space-y-3">
                            <DialogTitle class=" text-lg/8">Are you sure you want to delete <br />"{{ shareValueNameToDelete }}"?</DialogTitle>
                            <DialogDescription>
                                Once after deleted the shareValue, you can't able to do the recovery.
                            </DialogDescription>
                        </DialogHeader>
